...
(function(F, x) {
	...
	function N(a) {
		...
		for (var k =
			...
			h = new Date(G);
			A(h);
			p = Array.prototype.slice.call(b.querySelectorAll(".pmu-instance"))[k];
			n(b, "pmu-view-years") ? (w(h, 12 * (k - d)), m = h.getFullYear() - 6 + " - " + (h.getFullYear() + 5)) : n(b, "pmu-view-months") ? (w(h, k - d), m = h.getFullYear()) : n(b, "pmu-view-days") && (q(h, k - d), m = D(h, c.title_format, c.locales[c.locale]));
			if (!r && l && (r = new Date(h), c.select_day ? q(r, c.calendars - 1) : c.select_month ? w(r, c.calendars - 1) : w(r, 12 * (c.calendars - 1)), r > l)) {
				--k;
				q(G, -1);
				r = void 0;
				continue
			...
			var t = function(a) {
				...
				u = function(a, b) {
					var d = (new Date(e[0])).getFullYear(),
						h = (new Date(e[1])).getFullYear(),
						g = (new Date(e[0])).getMonth(),
						f = (new Date(e[1])).getMonth();
					return "range" == c.mode && (a > d && a < h || a > d && a == h && b <= f || a == d && a < h && b >= g || a == d && a == h && b >= g && b <= f) || "multiple" == c.mode && -1 !== e.reduce(function(a, c) {
						c = new Date(c);
			...
			(function() {
				var a = [],
					...
					f, k, l;
				for (l = 0; 12 > l; ++l) f = b + l, k = document.createElement("div"), k.textContent = f, k.__pickmeup_year = f, c.min &&
					f < d || c.max && f > e ? g(k, "pmu-disabled") : t(f) && g(k, "pmu-selected"), a.push(k);
			...
			(function() {
				var a = [],
					...
					k = (new Date(c.max)).getMonth(),
					l, m;
				for (l = 0; 12 > l; ++l) m = document.createElement("div"), m.textContent = c.locales[c.locale].monthsShort[l], m.__pickmeup_month = l, m.__pickmeup_year = b, c.min && (b < d || l < e && b == d) || c.max && (b > f || l > k && b >= f) ? g(m, "pmu-disabled") :
					u(b, l) && g(m, "pmu-selected"), a.push(m);
			...
			(function() {
				...
				(function() {
					h.setDate(1);
					var a = (h.getDay() - c.first_day) % 7;
					z(h, -(a + (0 > a ? 7 : 0)))
				})();
				for (e = 0; 42 > e; ++e) f = document.createElement("div"), f.textContent = h.getDate(), f.__pickmeup_day = h.getDate(), f.__pickmeup_month = h.getMonth(), f.__pickmeup_year = h.getFullYear(), b != h.getMonth() && g(f, "pmu-not-in-month"), 0 == h.getDay() ? g(f, "pmu-sunday") : 6 == h.getDay() &&
					g(f, "pmu-saturday"), k = c.render(new Date(h)) || {}, l = h.valueOf(), m = c.min && c.min > h || c.max && c.max < h, k.disabled || m ? g(f, "pmu-disabled") : (k.selected || c.date.valueOf() == l || c.date instanceof Array && c.date.reduce(function(a, b) {
						return a || l === b.valueOf()
					}, !1) || "range" == c.mode && l >= c.date[0] && l <= c.date[1]) && g(f, "pmu-selected"), l == d && g(f, "pmu-today"), k.class_name && k.class_name.split(" ").forEach(g.bind(f, f)), a.push(f), z(h, 1);
	...
	function u(a, b) {
		...
		a = a.split(d);
		for (var d = c.split(d), g, f, l, h, m, p = new Date, c = 0; c < a.length; c++) switch (d[c]) {
			case "b":
			...
			case "p":
				/pm/i.test(a[c]) &&
					12 > h ? h += 12 : /am/i.test(a[c]) && 12 <= h && (h -= 12);
				break;
			case "M":
				m = parseInt(a[c], 10)
		}
		e = new Date(void 0 === l ? p.getFullYear() : l, void 0 === f ? p.getMonth() : f, void 0 === g ? p.getDate() : g, void 0 === h ? p.getHours() : h, void 0 === m ? p.getMinutes() : m, 0);
	...
	function I(a, b) {
		...
		if (b || n(c, "pmu-hidden")) {
			...
			d.bound.fill();
			B(a, "input") && ((b =
				a.value) && d.bound.set_date(b), t(a, a, "keydown", function(a) {
			...
			if (C(a, "show") && !d.flat) {
				...
				m + c.offsetHeight > f + h && (m = e.top - c.offsetHeight);
	...
	function U(a, b) {
		...
		var c = a.__pickmeup.element;
		a = a.__pickmeup.options;
		n(c, "pmu-view-years") ? w(a.current, 12) :
			n(c, "pmu-view-months") ? w(a.current, 1) : n(c, "pmu-view-days") && q(a.current, 1);
		b && a.bound.fill()
	}
	...
	function W(a, b, c) {
		...
		if (!(b instanceof Array) || 0 < b.length)
			if (d.date = u(b, d), "single" != d.mode)
				for (d.date instanceof Array ? (d.date[0] = d.date[0] || u(new Date, d), "range" == d.mode &&
						(d.date[1] = d.date[1] || u(d.date[0], d))) : (d.date = [d.date], "range" == d.mode && d.date.push(u(d.date[0], d))), b = 0; b < d.date.length; ++b) d.date[b] = L(d.date[b], d.min, d.max);
		...
		c ? d.current = u(c, d) : (c = "single" ===
			d.mode ? d.date : d.date[d.date.length - 1], d.current = c ? new Date(c) : new Date);
		d.current.setDate(1);
		d.bound.fill();
	...
	function E(a, b) {
		...
		if (!a.__pickmeup) {
			...
			d.max && (d.max =
				u(d.max, d), d.select_day || d.max.setDate(1));
	...
	E.defaults = {
		...
		instance_template: function(a) {
			...
			return '\x3cdiv class\x3d"pmu-instance"\x3e\x3cnav\x3e\x3cdiv class\x3d"pmu-prev pmu-button"\x3e' + a.prev + '\x3c/div\x3e\x3cdiv class\x3d"pmu-month pmu-button"\x3e\x3c/div\x3e\x3cdiv class\x3d"pmu-next pmu-button"\x3e' +
